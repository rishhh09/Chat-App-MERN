    ...
    >
      <div className="flex items-center space-x-3">
        <div className="relative flex-shrink-0">
          <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center font-semibold text-sm ${
            chat.isGroup ? 'bg-purple-600' : 'bg-blue-600'
          }`}>
            {chat.isGroup ? <Users size={16} className="md:w-5 md:h-5" /> : chat.avatar}
          </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <h3 className="font-medium text-white truncate text-sm md:text-base">{chat.username}</h3>
            <span className="text-xs text-gray-500 flex-shrink-0">{chat.time}</span>
          </div>
          <div className="flex items-center justify-between">
            <p className="text-xs md:text-sm text-gray-400 truncate pr-2">{chat.lastMessage}</p>
            {chat.unread > 0 && (
...
const Sidebar = ({ conversations, selectedChat, setSelectedChat, searchTerm, setSearchTerm }) => {
  return (
    <div className="h-full bg-black border-r border-gray-800 flex flex-col">
      ...
      <div className="p-3 md:p-4 border-b border-gray-800 flex-shrink-0">
        <div className="flex items-center justify-between mb-3 md:mb-4">
          <h1 className="text-lg md:text-xl font-bold text-blue-400">ChatApp</h1>
          <div className="flex items-center space-x-1 md:space-x-2">
            <button className="p-1.5 md:p-2 hover:bg-gray-800 rounded-lg transition-colors">
              <Settings size={18} className="text-gray-400 md:w-5 md:h-5" />
            </button>
            <button className="p-1.5 md:p-2 hover:bg-gray-800 rounded-lg transition-colors">
        ...
        <div className="relative">
          <Search className="absolute left-3 top-2.5 md:top-3 text-gray-500" size={14} />
          <input
            type="text"
