...
const MessageBubble = ({ message, currentUserId }) => {
  ...
  return (
    <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} px-1`}>
      <div className={`max-w-[280px] sm:max-w-xs md:max-w-sm lg:max-w-md px-3 md:px-4 py-2 rounded-lg ${
        isOwn
        ...
        <p className={`text-xs mt-1 ${
          ...
          {message.createdAt ? new Date(message.createdAt).toLocaleTimeString() :  (message.time ?? '')}
        </p>
      </div>
    </div>
  );
